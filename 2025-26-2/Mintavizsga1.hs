...
pAtom = asum "pAtom: no atom matched" [
  ...
  ListLit <$> between (char' '[') (sepBy pExp (char' ',')) (char' ']'),
  Var <$> pNonKeyword,
...
statement = asum "statement: no statement matched" [sIf, sWhile, sAssign, sAssignAt]
...
sAssignAt = AssignAt <$> pNonKeyword <*> (pKeyword "!!" *> pExp) <*> (pKeyword ":=" *> pExp) 
