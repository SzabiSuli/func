...
-- Megaparsec
-- Haskell industry standard hiba kezelő vagy mi
...
-- Valóságban monadTransformerekkel maybe helyett ExceptT-vel van még definiálva state mellet
...
-- dropRest :: Parser a -> Parser ()
-- dropRest p = Parser $ do
--   (_, xs) <- runParser p
--   return ((), xs)
...
string = traverse_ char --traverse ami nem kombinálja az eredményeket
...
optional' :: Parser a -> Parser (Maybe a)
optional' p = (Just <$> p) <|> return Nothing -- becsomagolja a Nothingot Justba
...
-- [A-Z]{10}
p5 :: Parser ()
p5 = replicateM_ 10 (satisfy (\c -> c >= 'A' && c <= 'Z')) -- void $ _

-- \d{2}
p6 :: Parser ()
p6 = replicateM_ 2 $ satisfy isDigit
