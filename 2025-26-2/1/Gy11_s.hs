...
program = many statement -- topLevel in parseProgram

statement :: Parser Statement
statement = tok (sIf <|> sWhile <|> sAssign) <* char' ';'

sIf :: Parser Statement                          -- <*> should be right before the next parser that has a relevant result
sIf = If <$> (string' "if " *> pExp) <* string' "then "  <*> many statement <* string' "end"
-- added spacebar after keywords so thenhalf := 2 is not valid

sWhile :: Parser Statement
sWhile = While <$> (string' "while " *> pExp) <* string' "do " <*> many statement <* string' "end"
...
evalExp (IntLit i) env = pure $ VInt i
evalExp (FloatLit f) env = pure $ VFloat f
evalExp (BoolLit f) env = pure $ VBool f
evalExp (Var var) env = do 
