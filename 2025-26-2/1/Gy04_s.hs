...
magicFunctionDo x = do
    y <- incrementIfEven x -- incrementIfEven x >>= \y -> _
    z <- combineThrees (*) x y
    incrementIfEven z
...
-- putStrLn "alma" >>= \_ -> putStrLn "fa"
-- ehelyett        ^^^^^^^^^
-- putStrLn "alma" >> putStrLn "fa"

-- readLn >>= \x -> return (x + 1)
-- egy egész típusosztály lehet, de kell neki egy konkrét típus
-- ebben az esetben Int-re defaultol


printAll :: Show a => [a] -> IO ()
-- printAll = foldMap (putStrLn . show)
printAll = foldMap print
...
readAndAdd' :: (Read a, Num a) => [a] -> IO [a]
readAndAdd' = traverse fn where
    fn x = do 
        y <- readLn
        return $ x + y

readAndAdd'' :: (Read a, Num a) => [a] -> IO [a]
readAndAdd'' = traverse (\x -> readLn >>= \y -> return $ x + y)
