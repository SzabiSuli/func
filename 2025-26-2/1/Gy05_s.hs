...
-- nem pont így van megadva, ezért state-et használunk State helyett
...
pop :: State [a] (Maybe a)
-- pop = state helper where
  -- helper :: [a] -> (Maybe a, [a])
  -- helper [] = (Nothing, [])
  -- helper (x:xs) = (Just x, xs)

-- lehet put ot is használni
pop = 
  get >>= \case
    [] -> return Nothing
    (x : xs) -> 
...
takeK :: Int -> State [a] [a]
-- takeK = state . helper where
--   helper :: Int -> [a] -> ([a], [a])
--   helper _ [] = ([], [])
--   helper 0 xs = ([] , xs)
--   helper n (x:xs) = 
--     let (ys , zs) = helper (n-1) xs 
--     in (x : ys, zs)
takeK 0 = return []
takeK n = 
  get >>= \case
...
popLast = 
  get >>= \case
    ...
    (x:xs) ->
      put xs >>
      popLast >>= \case
        Nothing -> return (Just x) --üresen hagyjuk a state-t
        jy@(Just y) -> 
          get >>= \ys ->
          put (x : ys) >>
          return jy
...
safeDiv n 0 = Left DivByZeroError
...
safeDivE n 0 = throwError DivByZeroError
safeDivE n m = return (div n m)
...
safeDivF n m = catchError (safeDivE n m) $ \case
