{-# LANGUAGE DerivingVia, DerivingStrategies, LambdaCase #-}
...
homeDirOf user ((u, info) : es)
  | user == u = Just $ getHomeDirectory info
  | otherwise = homeDirOf user es
...
homeDirOfRM s = 
  ask >>= \case
    ...
    (_ : xs) -> local (const xs) $ homeDirOfRM s
...
getAdminHomesRM = 
  ...
  ((u, ui) : xs) 
    | isAdmin ui ->
      local (const xs) getAdminHomesRM >>= \ys ->
...
labelListR (x:xs) = 
  ask >>= \i ->
  -- local (const (i + 1)) (labelListR xs) >>= \ys ->
  local (+1) (labelListR xs) >>= \ys ->
  return $ (i, x) : ys
...
filterWithIndexR f (x:xs) =
  ask >>= \i ->
    if f i x 
    then local (+1) $ filterWithIndexR f xs >>= \ys ->
      return $ x : ys
    else local (+1) $ filterWithIndexR f xs
...
apiRequest s = (s == "localhost", ["apiRequest"])
...
nameserver x
  | even x = 
    let (_, qa) = apiRequest "1.0.0.1"
    in (div x 2, qa ++ ["nameserver"])
