...
printHomeDirIfAdmin = do
  MkEnv admin hd <- ask -- MonadReader r m => m r
  when admin $ tell [hd] -- when ha igazat kap lefuttatja a monádot, különben ()-t returnol
...
-- addHomeIfNotIn :: ReaderT Env (State)
addHomeIfNotIn :: (MonadState FileSystem m, MonadReader Env m) => m ()
addHomeIfNotIn = do
  s <- get
  MkEnv _ hd <- ask
  if contains hd s
...
undupe = do
  s <- get 
  let unduped = nub s
  let duplicates = s \\ unduped
  -- in  -- no in with do notation
  tell duplicates
  put unduped
...
-- io legyen legbelül
-- utána közvetlen except
-- utána mindegy a sorrend
...
type M a = ReaderT Env (WriterT [String] (StateT FileSystem (ExceptT FSError IO))) a

-- runExceptT (runStateT (runWriterT $ runReaderT f1 (MkEnv True "hoem/j")) basicExecutables )

-- f1 :: StateT FileSystem (ExceptT FSError IO) ()
f1 :: M ()
f1 = do
  path <- liftIO getLine
  if not $ any (== []) (splitOn '/' path)
  then modify (path:)
  else throwError BadPath
