...
-- natural = (\l -> sum zipWith (*) reverse l...) <$> atLeastOneDigit
...
rightAssoc f a s = foldr1 f <$> sepBy1 a s
--   xs <- sepBy a s
  -- pure $ foldr f xs 

-- Ugyanaz mint a rightAssoc csak balra
leftAssoc :: (a -> a -> a) -> Parser a -> Parser sep -> Parser a
leftAssoc f a s = foldl1 f <$> sepBy1 a s
...
pSharp :: Parser Exp
pSharp = leftAssoc (:#) pPow (char' '#')

pDiv :: Parser Exp
pDiv = leftAssoc (:/) pMul (char' '/')

pSub :: Parser Exp
pSub = leftAssoc (:-) pAdd (char' '-')
